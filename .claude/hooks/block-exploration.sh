...
if echo "$CMD" | grep -qE '^\s*(ls\s*$|ls\s+-[a-zA-Z]+\s*$|find\s+[./]|tree\s*$|tree\s+-|du\s+-|locate\s)'; then
  echo "BLOCKED (Rule 1): Autonomous filesystem exploration forbidden. <system_warning>Use mcp__serena__get_symbols_overview, mcp__serena__find_symbol, mcp__serena__find_referencing_symbols or mcp__serena__find_file / mcp__serena__list_dir instead. If Serena not available, use Grep/Read/Glob tools.</system_warning>" >&2
  exit 2
fi
